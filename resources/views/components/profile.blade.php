<x-layout>
    <div class="container py-md-5 container--narrow">
        <h2>
          ...
          <a href="/manage-avatar"><img class="avatar-small" src="{{$sharedData['avatar']}}" /></a>{{$sharedData['username']}}
          @else
          <img class="avatar-small" src="{{$sharedData['avatar']}}" /> {{$sharedData['username']}}
            @endif
            @auth
            @if(!$sharedData['userFollowed'] && auth()->user()->username != $sharedData['username'])
